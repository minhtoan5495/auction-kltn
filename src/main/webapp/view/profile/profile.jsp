			...
			<div class="panel-body profile_body_item">
				<c:forEach var="auction" items="${auctions}">
					<div class="row"
						...
						<c:choose>
							<c:when test="${auction.auction.endTime >=now }">
								<div class="col-sm-3">
									<img alt=""
								...
								<div class="col-sm-6">
									<strong> ${auction.auction.product.name} </strong><br> <label
										...
										<p style="color: #59b210; font-weight: 400">
											Ngày kết thúc
											<fmt:formatDate value="${auction.auction.endTime}"
												pattern="dd-MM-yyyy" />
										</p>
										<p style="color: #ff4f4f; font-weight: 400">
											Giá tiền:
											<fmt:formatNumber value="${auction.auction.product.price}"
												type="number" />
											đ
										</p>
			...
			<div class="panel-body profile_body_item">
				<c:forEach var="auction" items="${auctions}">
					<div class="row"
						style="border: 1px solid gray; margin-left: 5px; margin-right: 5px; margin-bottom: 5px;">
						<c:choose>
							<c:when test="${auction.auction.endTime <= now and not empty auction}">
								<div class="col-sm-3">
									<img alt=""
										src="${pageContext.request.contextPath }/resource/images/product/${auction.auction.product.images.get(i).getImageUrl()}">
								</div>
